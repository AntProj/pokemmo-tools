...
- **Rail-system / off-grid entities have null world coords.** ~77 warps (concentrated on Victory Road and other rail-served zones) and ~65 misc actors have `worldX/worldY/worldZ = null` in the raw data — they live on the train graph or in off-map actor systems rather than at tile positions. The generator detects this via `Number.isFinite()` and emits `pixelX: null, pixelY: null, offMap: true` on those entries. The React side skips them as map markers (otherwise they'd all stack at one degenerate pixel) and surfaces them in a separate "Off-map warps" sidebar panel that still lets the user navigate to the destination map.
