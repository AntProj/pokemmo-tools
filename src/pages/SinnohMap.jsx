...
const MAPS_INDEX_URL    = asset('data/maps/sinnoh/maps-index.json');
const OVERWORLD_LOC_URL = asset('data/maps/sinnoh/overworld-locations.json');
const ZONE_GRAPH_URL    = asset('data/maps/sinnoh/zone-graph.json');
...
export default function SinnohMap({ theme, onTheme }) {
  ...
  // Cross-zone route state lives at SinnohMap level (not MapView) so it
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(OVERWORLD_LOC_URL).then(r => r.json()),
      fetch(ZONE_GRAPH_URL).then(r => r.json()),
    ]).then(([idx, loc, graph]) => {
      if (cancelled) return;
      setMapsIndex(idx);
    ...
    <main className="max-w-7xl mx-auto px-4 py-4 space-y-4">
      ...
      {mapsIndex && (
        <MapView
          ...
          onNavigateToZone={(id) => navigate(`/map/${id}`)}
        />
      )}
    </main>
  );
...
function ZoneJumpPicker({ mapsIndex, zoneId, onChange }) {
  ...
  const options = useMemo(() => {
    ...
    return [{ id: 'world', name: 'Sinnoh Overworld', type: 'overview', label: 'Sinnoh Overworld' }, ...detail];
  }, [mapsIndex]);
  ...
  if (!entry) {
    return (
      <div className="rounded-md border border-amber-300 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 p-3 text-sm text-amber-800 dark:text-amber-200">
        Unknown zone "{zoneId}". <Link to="/map" className="underline">Back to overworld</Link>.
      </div>
    );
  }
  if (loadError) {
